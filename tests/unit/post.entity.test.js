...
describe('Post Entity', () => {
  ...
  it('should throw an error if title is missing', () => {
    expect(() => new Post('1', null, 'Test Content', [], 'author1')).toThrow(
      'Post must have a title, content, and authorId'
    );
  });

  it('should throw an error if content is missing', () => {
    expect(() => new Post('1', 'Test Title', null, [], 'author1')).toThrow(
      'Post must have a title, content, and authorId'
    );
  });

  it('should throw an error if authorId is missing', () => {
    expect(() => new Post('1', 'Test Title', 'Test Content', [], null)).toThrow(
      'Post must have a title, content, and authorId'
    );
  });
});
